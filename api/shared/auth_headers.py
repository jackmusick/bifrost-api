"""
Authentication Header Derivation and Scope Context
Provides consistent scoping logic for all endpoints with organization context
"""
...
def get_scope_context(req: func.HttpRequest) -> Tuple[Optional[str], Optional[str], Optional[func.HttpResponse]]:
    """
    **CANONICAL FUNCTION FOR ALL SCOPED DATA ACCESS**

    Get organization scope context with consistent security enforcement.

    SCOPING RULES:
    1. Platform Admins:
       - Can pass X-Organization-Id header to impersonate an org
       - If no X-Organization-Id header → org_id = None (GLOBAL scope)
       - org_id = None → PartitionKey = "GLOBAL" in database

    2. Regular Users:
       - org_id ALWAYS derived from database (their assigned org)
       - Cannot pass X-Organization-Id header (403 if attempted)
       - org_id is NEVER None

    3. Database Operations:
       - org_id = None → use PartitionKey = "GLOBAL"
       - org_id = "some-org" → use PartitionKey = "some-org"

    4. Workflow Engine:
       - Management API sends X-Organization-Id header only if org_id is not None
       - No header → workflow engine treats as GLOBAL scope
    ...
    Usage:
        ...
        # Use org_id for database operations
        partition_key = org_id or "GLOBAL"
        table.query_by_org(partition_key)
    """
    try:
        ...
        if user_is_admin:
            if provided_org_id:
                # Admin is impersonating an org
                logger.info(f"PlatformAdmin {user.email} operating in org scope: {provided_org_id}")
                return provided_org_id, user.user_id, None
            else:
                # Admin in GLOBAL scope (no org specified)
                logger.info(f"PlatformAdmin {user.email} operating in GLOBAL scope")
                return None, user.user_id, None

        # Regular User Logic
        if provided_org_id:
            # Non-admin trying to set org context → 403
            logger.warning(f"Non-admin user {user.email} attempted to set X-Organization-Id header")
            error = ErrorResponse(
                error="Forbidden",
                message="Only platform administrators can override organization context"
            )
            return None, None, func.HttpResponse(
                json.dumps(error.model_dump()),
                status_code=403,
        ...
        # Derive org from database for regular user
        ...
        logger.info(f"User {user.email} operating in org scope: {org_id}")
        return org_id, user_id, None

    except Exception as e:
        logger.error(f"Error getting scope context: {str(e)}", exc_info=True)
        error = ErrorResponse(
            error="InternalServerError",
            message="Failed to determine scope context"
        )
        return None, None, func.HttpResponse(
            json.dumps(error.model_dump()),
            status_code=500,
