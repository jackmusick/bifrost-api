...
class TestEnsureUserExistsInDb:
    ...
    @pytest.mark.asyncio
    async def test_existing_user_updates_last_login(self, mock_user_repo):
        """Existing user should have last login updated"""
        # Arrange
        existing_user = Mock(spec=User)
        existing_user.email = "user@example.com"
        mock_user_repo.get_user = AsyncMock(return_value=existing_user)
        mock_user_repo.update_last_login = AsyncMock()

        # Act
        await ensure_user_exists_in_db("user@example.com", is_platform_admin=False)
    ...
    @pytest.mark.asyncio
    async def test_creates_platform_admin_user(self, mock_user_repo):
        ...
        created_user.email = "admin@example.com"
        mock_user_repo.create_user = AsyncMock(return_value=created_user)

        # Act
        await ensure_user_exists_in_db("admin@example.com", is_platform_admin=True)
    ...
    @pytest.mark.asyncio
    async def test_creates_org_user(self, mock_user_repo):
    ...
    @pytest.mark.asyncio
    async def test_org_user_auto_provisions_relationship(self, mock_user_repo):
        """New org user should attempt domain-based org relationship provisioning"""
        # Arrange
        mock_user_repo.get_user = AsyncMock(return_value=None)

        created_user = Mock(spec=User)
        created_user.email = "user@acme.com"
        mock_user_repo.create_user = AsyncMock(return_value=created_user)

        # Act
        with patch("shared.user_provisioning._provision_org_relationship_by_domain") as mock_provision:
            mock_provision.return_value = "org-123"
            await ensure_user_exists_in_db("user@acme.com", is_platform_admin=False)

            # Assert
            mock_provision.assert_called_once_with("user@acme.com")

    @pytest.mark.asyncio
    async def test_org_user_handles_provision_failure_gracefully(self, mock_user_repo):
        """If org relationship provisioning fails, should log but not raise"""
        # Arrange
        mock_user_repo.get_user = AsyncMock(return_value=None)

        created_user = Mock(spec=User)
        created_user.email = "user@nowhere.com"
        mock_user_repo.create_user = AsyncMock(return_value=created_user)

        # Act - should not raise
        with patch("shared.user_provisioning._provision_org_relationship_by_domain") as mock_provision:
            mock_provision.side_effect = ValueError("No matching domain")
            await ensure_user_exists_in_db("user@nowhere.com", is_platform_admin=False)
...
class TestGetUserOrganization:
    ...
    @pytest.mark.asyncio
    async def test_returns_org_id_for_user(self, mock_user_repo):
        """Should return org_id when user has assignment"""
        # Arrange
        mock_user_repo.get_user_org_id = AsyncMock(return_value="org-123")

        # Act
        result = await get_user_organization("user@example.com")
    ...
    @pytest.mark.asyncio
    async def test_returns_none_for_no_org(self, mock_user_repo):
    ...
    @pytest.mark.asyncio
    async def test_handles_query_error(self, mock_user_repo):
