...
def list_forms(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Query Forms table for both org-specific and global forms
...
def get_form(req: func.HttpRequest) -> func.HttpResponse:
    """
    GET /api/forms/{formId}?orgId={orgId}
    ...
    logger.info(f"User {user.email} retrieving form {form_id}")
    ...
    try:
        ...
        # Try to get form from org partition first, then GLOBAL
        forms_service = TableStorageService("Forms")
...
def update_form(req: func.HttpRequest) -> func.HttpResponse:
    """
    PUT /api/forms/{formId}?orgId={orgId}
    Update a form
...
def delete_form(req: func.HttpRequest) -> func.HttpResponse:
    """
    DELETE /api/forms/{formId}?orgId={orgId}
...
def submit_form(req: func.HttpRequest) -> func.HttpResponse:
    """
    POST /api/forms/{formId}/submit?orgId={orgId}
    ...
    logger.info(f"User {user.email} submitting form {form_id}")
    ...
    try:
        ...
        # Call workflow engine to execute workflow
        workflow_engine_url = "http://localhost:7072"
        ...
        # Execute workflow
        response = requests.post(workflow_url, json=workflow_payload, headers=headers)
