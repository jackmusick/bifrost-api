...
vi.mock("@/contexts/AuthContext", () => ({
	useAuth: () => ({ user: { id: "sidebar-test-user" } }),
}));
...
describe("SidebarCollections", () => {
	...
	it("keeps empty collections manageable but hides empty shared collections from readers", () => {
		...
		renderWithProviders(<SidebarCollections isCollapsed={false} />);
		expect(
			screen.getByRole("link", { name: "My empty collection" }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("link", { name: "Useful shared collection" }),
		).toBeInTheDocument();
		expect(
			screen.queryByText("Empty shared collection"),
		).not.toBeInTheDocument();
	});
});
