...
export function ExpressionEditor({
  ...
  // Calculate height based on content (single line ~40px, then grows)
  const lineCount = (value || '').split('\n').length
  const calculatedHeight = height ?? Math.max(40, Math.min(lineCount * 20 + 24, 200))

  const validateExpression = (expr: string): { isValid: boolean; error?: string } => {
    if (!expr.trim()) {
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      <div className={cn("border rounded-md overflow-hidden", validationError && "border-destructive")}>
        <div className="px-3">
          <Editor
      ...
      {!validationError && !helpText && (
        <p className="text-xs text-muted-foreground">{defaultHelpText}</p>
      )}
    </div>
  )
}
...
export function validateExpression(expr: string): { isValid: boolean; error?: string } {
  if (!expr.trim()) {
    return { isValid: true }
  }
