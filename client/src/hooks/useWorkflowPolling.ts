    ...
    const pollExecutions = async () => {
      try {
        ...
        const results = await Promise.allSettled(
          executionIds.map((id) => executionsService.getExecution(id))
        )
          ...
          if (result.status === 'fulfilled' && result.value) {
            ...
            setState((prev) => {
              ...
              if (prevStatus !== execStatus) {
                if (
                  (execStatus === 'Success' || execStatus === 'CompletedWithErrors') &&
                  callbacksRef.current.onExecutionComplete
                ) {
                  callbacksRef.current.onExecutionComplete(execution)
                } else if (execStatus === 'Failed' && callbacksRef.current.onExecutionFail) {
                  callbacksRef.current.onExecutionFail(execution)
                }
              }
