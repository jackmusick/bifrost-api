...
// Define local types matching the service types
type SecretCreateRequest = { name: string; value: string; description?: string };
type SecretUpdateRequest = { value: string; description?: string };
...
export function useCreateSecret() {
	...
	return useMutation({
		mutationFn: (data: SecretCreateRequest) =>
...
export function useDeleteSecret() {
	...
	return useMutation({
		mutationFn: (secretName: string) =>
			secretsService.deleteSecret(secretName),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["secrets"] });
