 ...
 */
export function useLaunchWorkflow({ form, workflowParams = {} }: UseLaunchWorkflowOptions) {
  const { context, setWorkflowResults, setIsLoadingLaunchWorkflow } = useFormContext()

  // Memoize serialized objects for dependency comparison
  const serializedQuery = useMemo(() => JSON.stringify(context.query), [context.query])
  const serializedParams = useMemo(() => JSON.stringify(workflowParams), [workflowParams])

  useEffect(() => {
    // Only execute if form has a launch workflow configured
    if (!form.launchWorkflowId) {
      return
    }
    ...
    const executeLaunchWorkflow = async () => {
      try {
        ...
        const result = await formsService.executeFormStartup(
          form.id,
          inputData
        )

        // Extract workflow result and update context
        const workflowOutput = (result.result as Record<string, unknown>) || {}
        setWorkflowResults(workflowOutput)
      } catch (error) {
        console.error('Failed to execute launch workflow:', error)

        // Extract error message from response
        const errorResponse = error as { response?: { data?: { message?: string } } } & Error
        const errorMessage = errorResponse?.response?.data?.message || errorResponse?.message || 'Failed to load form data'
