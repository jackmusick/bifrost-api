...
def execute_workflow(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        # Derive or validate auth headers
        org_id, user_id, error = get_auth_headers(req)
        if error:
            return error

        logger.info(f"Executing workflow with context: org={org_id}, user={user_id}")
...
def get_data_provider(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        logger.info(f"Fetching data provider with context: org={org_id}, user={user_id}")
