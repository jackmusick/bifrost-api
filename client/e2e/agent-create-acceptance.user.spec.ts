...
test("[AGENT-01 desktop] member creates private agent and reopens persisted settings", async ({
	page,
	api,
}) => {
	...
	try {
		...
		await expect(
			page.getByText("Private agents are available only to their owner."),
		).toBeVisible();
